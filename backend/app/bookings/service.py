...
async def pay_booking(
    ...
    if booking.expires_at and booking.expires_at < datetime.now(UTC):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Reservation expired")
    ...
    # Charge
    charge_result: PaymentResult = await payment_gateway.charge(
    ...
    if charge_result.status == "success":
        ...
        await log_audit(
            ...
            "reserved",
            "cancelled",
            metadata={"failure_reason": charge_result.failure_reason},
            ip_address=ip_address,
        )
